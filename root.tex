...
Similar to before, we construct a set $\mathbb{O}^D_{N_P}$ defined as:
  ...
  To calculate the input feasible set $\mathbb{O}_{N_P}$ by solving the equations in \eqref{overall}, we need the initial state $x(t)$ of the system \eqref{ssmodel}. This initial state includes $x_D(t)$. We can use this to first calculate the corresponding disturbance sequence set $\mathbb{W}$ to calculate the set $\mathbb{O}_{N_P}$ as shown in \eqref{overall}. This can then be used in the MPC scheme \eqref{RMPC} to calculate the optimal control input $\bar{u}$. This procedure is summarized in Algorithm \ref{alg:mpc_wLP}.
